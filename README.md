...
- 修复退出登录后首页仍显示个人数据、重新登录后又无法恢复的问题。现在退出时仅清除 Cookie 和钥匙串会话，首页会隐藏个人内容；重新登录后会读取 NGA 账号的服务端收藏版块并恢复本地阅读历史。
- 首页明确区分「收藏版块」与「收藏主题」：收藏主题会遍历服务端收藏夹及分页，首页显示总篇数、收藏夹数和前三条，并可进入完整列表。
- 将退出操作文案调整为「退出登录」，并补充本地数据保留说明。
...
- **我的**：会话状态、网页验证 / 登录 NGA 账号、退出登录（保留本地阅读记录与收藏）、关于。
